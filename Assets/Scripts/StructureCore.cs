...
public class StructureCore : MonoBehaviour
{
   ...
   // public bool autocaptureMode = false;
    ...
    void Start()
    {
        //TODO: make this something that only happens when generating a depth frame!!!!
        GameObject[] gos;
        gos = GameObject.FindGameObjectsWithTag("glass");
    ...
    void Update()
    {
        switch (state)
        {
            case 0:
                ...
                //randomly select the next pattern we are going to use
                int i = Random.Range(0, patternTextures.Length);
                patternTexture = patternTextures[i];
                maskTexture = maskTextures[i];

                //enable dot-pattern projector
            ...
            case 3:
                ...
                //randomly select the next pattern we are going to use
                int ind = Random.Range(0, patternTextures.Length);
                patternTexture = patternTextures[ind];
                maskTexture = maskTextures[ind];

                //setup projector to produce dot-pattern
                projector.cookie = patternTexture;
                //TODO: enable auto exposure
                camLeft.SetVolumeLayerMask(postProcessingMaskFull);
                camRight.SetVolumeLayerMask(postProcessingMaskFull);
